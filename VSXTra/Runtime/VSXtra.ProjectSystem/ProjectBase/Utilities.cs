...
using VSXtra.ProjectSystem.Properties;
...
namespace VSXtra.ProjectSystem
{
  // ================================================================================================
  /// <summary>
  /// This static class is a container for many helper and utility methods used withtin the Managed
  /// Project System.
  /// </summary>
  // ================================================================================================
  public static class Utilities
  {
    private const string defaultMSBuildVersion = "3.5";
    private static readonly char[] curlyBraces = new[] {'{', '}'};

    // --------------------------------------------------------------------------------------------
    /// <summary>
    /// Look in the registry under the current hive for the path of MSBuild and return thi path
    /// </summary>
    /// <returns>MSBuild path in the current registry hive.</returns>
    // --------------------------------------------------------------------------------------------
    [CLSCompliant(false)]
    ...
    {
      string msBuildPath;
      using (var root = VSRegistry.RegistryRoot(
        serviceProvider,
      ...
      string registryPath = string.Format(
        CultureInfo.InvariantCulture,
        "Software\\Microsoft\\MSBuild\\ToolsVersions\\{0}",
        version);
      ...
      if (string.IsNullOrEmpty(msBuildPath))
      {
        throw new FileLoadException(Resources.ErrorMsBuildRegistration);
      }
      return msBuildPath;
    }
    ...
    {
      var selectionMonitor = site.GetService<IVsMonitorSelection>();
      uint cookie;
      int active;
      Guid designContext = VSConstants.UICONTEXT_DesignMode;
      ErrorHandler.ThrowOnFailure(selectionMonitor.GetCmdUIContextCookie(ref designContext,
                                                                         out cookie));
    ...
    public static void ValidateFileName(IServiceProvider serviceProvider, string filePath)
    {
      ...
      else if (filePath.Length > NativeMethods.MAX_PATH)
      {
        errorMessage = String.Format(CultureInfo.CurrentCulture,
                                     SR.GetString(SR.PathTooLong, CultureInfo.CurrentUICulture),
                                     filePath);
      }
      ...
      if (errorMessage.Length > 0)
      {
        ...
        if (!IsInAutomationFunction(serviceProvider))
        {
          ...
          VsShellUtilities.ShowMessageBox(serviceProvider, title, errorMessage, icon, buttons,
                                          defaultButton);
        }
        else
        {
    ...
    internal static string GetActiveConfigurationName(Project automationObject)
    {
    ...
    public static bool ContainsInvalidFileNameChars(string name)
    {
      ...
      if (uri.IsFile)
      {
        ...
        if (segments != null && segments.Length > 0)
        {
          ...
          if (IsFileNameAllGivenCharacter('.', filePart) ||
    ...
    internal static Microsoft.Build.BuildEngine.Project InitializeMsBuildProject(Engine buildEngine,
                                                                                 string
                                                                                   fullProjectPath)
    {
      if (buildEngine == null)
      ...
      if (String.IsNullOrEmpty(fullProjectPath))
      {
        throw new ArgumentException(
          SR.GetString(SR.InvalidParameter, CultureInfo.CurrentUICulture), "fullProjectPath");
      }

      // Check if the project already has been loaded with the fullProjectPath. If yes return the build project associated to it.
      Microsoft.Build.BuildEngine.Project buildProject =
        buildEngine.GetLoadedProject(fullProjectPath);
    ...
    /// <returns>A loaded msbuild project.</returns>
    internal static Microsoft.Build.BuildEngine.Project ReinitializeMsBuildProject(
      Engine buildEngine, string fullProjectPath,
      Microsoft.Build.BuildEngine.Project exitingBuildProject)
    {
      // If we have a build project that has been loaded with another file unload it.
      try
      {
        if (exitingBuildProject != null && exitingBuildProject.ParentEngine != null &&
            !NativeMethods.IsSamePath(exitingBuildProject.FullFileName, fullProjectPath))
        {
    ...
    internal static Engine InitializeMsBuildEngine(Engine existingEngine,
    ...
    {
      object[] attributes =
        typeof (T).GetCustomAttributes(typeof (PropertyPageTypeConverterAttribute), true);
                                                              ...
                                                              out string platform)
    {
      ...
      ErrorHandler.ThrowOnFailure(solutionBuildManager.FindActiveProjectCfg(IntPtr.Zero, IntPtr.Zero,
                                                                            hierarchy, activeConfigs));
      ...
      return ProjectConfig.TrySplitConfigurationCanonicalName(canonicalName, out configuration,
                                                              out platform);
    }
    ...
    internal static bool IsShellInCommandLineMode(IServiceProvider serviceProvider)
    {
      ...
      ErrorHandler.ThrowOnFailure(shell.GetProperty(
    ...
    internal static void SaveDialogStateInSolution(IServiceProvider serviceProvider,
                                                   _ProjectLoadSecurityDialogState
                                                     projectLoadSecurityDialogState)
    {
      if (serviceProvider == null)
      {
        throw new ArgumentNullException("serviceProvider");
      ...
      ErrorHandler.ThrowOnFailure(
        solution.SetProperty((int) __VSPROPID2.VSPROPID_ProjectLoadSecurityDialogState,
                             projectLoadSecurityDialogState));
    }
    ...
    internal static bool IsProjectLocationSecure(string location)
    {
      ...
      var utilities = new Microsoft.VisualStudio.Build.ComInteropWrapper.Utilities();
